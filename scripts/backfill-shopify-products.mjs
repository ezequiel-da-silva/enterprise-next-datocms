#!/usr/bin/env node
/**
 * Backfill de `product_page` no Dato para produtos que já existiam antes das
 * subscrições de webhook (`products/create` / `products/update` só disparam em
 * alterações futuras).
 *
 * Lista os produtos pela Storefront API e reenvia cada um como um webhook
 * assinado para `/api/webhooks/shopify` — mesmo caminho que a Shopify usa, por
 * isso o upsert + publish é exatamente o de produção.
 *
 * Só produtos publicados no canal **Headless** aparecem na Storefront API.
 *
 * Uso:
 *   node scripts/backfill-shopify-products.mjs --url https://site.vercel.app
 *   npm run shopify:backfill -- --url https://site.vercel.app --dry-run
 */
...
function loadEnvFile() {
  try {
    ...
    // .env é opcional quando as variáveis já estão no ambiente
  }
}
...
async function listStorefrontProducts(domain, token) {
  const query = `
...
async function sendSignedWebhook({ endpoint, secret, domain, product }) {
...
async function main() {
  ...
  const products = await listStorefrontProducts(domain, storefrontToken);
  console.log(`backfill — ${products.length} produto(s) no canal Headless de ${domain}`);

  if (args.dryRun) {
    for (const product of products) console.log(`  [dry-run] ${product.handle} — ${product.title}`);
    return;
  }
  ...
  for (const product of products) {
    const { status, text } = await sendSignedWebhook({ endpoint, secret, domain, product });
    if (status === 200) {
      console.log(`  ok   ${product.handle}`);
    } else {
      failures += 1;
      console.error(`  FAIL ${product.handle} — ${status} ${text.slice(0, 120)}`);
    }
    await new Promise((resolve) => setTimeout(resolve, DELAY_MS));
  }

  console.log(`backfill — concluído (${products.length - failures} ok, ${failures} falha(s))`);
